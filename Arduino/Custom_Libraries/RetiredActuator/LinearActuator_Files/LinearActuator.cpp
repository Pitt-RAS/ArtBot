 ...
 *	@Return: a boolean value of whether or not the actuator is still moving
 *
 *  This function sends the actuator to a specified distance at max speed
 */
bool LinearActuator::sendToPos(int finalPos) {
 ...
 */
 bool LinearActuator::sendToPosWithSpeed(int finalPos, int finalSpeed) {
  ...
  //If we're below, extend the actuator
  if(currentPos < finalPos) {
      currentSpeed = -_A * pow((currentPos - _middlePos),2) + finalSpeed + 100;
      if(currentSpeed > 255) {
        currentSpeed = 255;
      }      
      digitalWrite(In1, LOW);
